...
**Combat Feel & Feedback: kill-fight timer removal, wound recovery correction + visibility, post-fight rewards summary**

**What was built:**
- Brainstormed and wrote `docs/superpowers/specs/2026-07-04-combat-feel-and-feedback-design.md` (first sub-project of the 04 Jul audit's 7-group sequence), then `docs/superpowers/plans/2026-07-05-combat-feel-and-feedback.md` (5 tasks), executed via Subagent-Driven Development in an isolated worktree
- `ui/viewmodel/UiModels.kt`, `ui/viewmodel/BandViewModel.kt`, `ui/screen/MissionsScreen.kt`: threaded a new `EncounterDetail.objective` field through to `BattleInProgressCard`; the "MM:SS remaining" countdown is now hidden for `objective == "kill"` fights (outcome is already fully computed the instant the fight starts — the countdown was just spoiling a guaranteed loss), still shown for survive/retrieve fights where racing a clock is the point. No change to the HP-bar tick animation or `EncounterEngine` itself.
- `worker/EncounterWorker.kt`, `design/master-design.md` §6.8: corrected wound recovery durations — both the design doc's original 1hr/2hr/6hr figures and the code's unrelated seconds-scale test placeholders were wrong. Real values: 18 min light, 30 min heavy, 2 hr pre-HoH safety-net (kept deliberately harsh — a real penalty for under-provisioning, not a nudge toward HoH, since starting areas have no HoH recipe at all). Locked in with a `WoundResolutionTest` case since these values had already been wrong once.
- `ui/screen/HouseOfHealingScreen.kt`: light/heavy ("Wounded") members no longer get offered HoH treatment options (contradicted the "time only, no food or prep" rule) — now show a live "Resting — back in Xm" countdown instead. Grievous members' existing craft-and-treat flow is untouched.
- `data/db/CombatReport.kt`, `data/db/Migration19To20.kt` (new), `data/db/HearthCraftDatabase.kt`, `di/DatabaseModule.kt`, `worker/EncounterWorker.kt`: additive migration (v19→v20) adding `moneyGranted`/`ingredientsGrantedJson`/`grimoireIdsGrantedJson`/`xpGranted` to `CombatReport`; `EncounterWorker` now captures the money/ingredients/grimoires/XP it was already silently granting onto the already-saved report (fetch-copy-save pattern, since the report row exists before this worker runs).
- `ui/screen/MissionsScreen.kt`, `ui/viewmodel/BandViewModel.kt`: post-fight recap card now shows a "Rewards" section (gold, ingredients by name, XP, grimoire found — visually distinct) reading the new fields; nothing shown on Defeat, matching the existing no-reward-on-defeat rule. New `BandViewModel.ingredientName()`/`grimoireName()` lookup helpers.

**Decisions made:**
- Wound recovery matrix (light/heavy flat-duration vs. grievous's five wound types with summed floor durations) fully documented in the design spec for completeness, but the grievous wound-type mechanic itself is unchanged — pre-existing design from the 2026-07-02 HoH mechanics spec, not new work this session
- Kill-fight timer removal is display-only — explicitly not a fast-forward/skip mechanic, not a change to how the fight is computed or animated; Wes was clear he wants the fight to "look as normal," just without the giveaway countdown
- Safety-net wound duration (2hr) is deliberately punitive, not a soft nudge — Wes corrected this framing mid-session (it's a real consequence for poor band management, and starting areas have no HoH recipe at all so it's the only consequence a new player faces)

**Anything that diverged from design/master-design.md:**
- §6.8 durations corrected (1hr/2hr/6hr → 18min/30min/2hr) — doc updated in the same commit as the code fix, per usual practice

**Coming up:**
- Next session: Wes picks the next of the 6 remaining audit sub-project groups (damage types recommended first — several other deferred items depend on it)
- Near term: same long-standing items as before (bio-stage triggers, `grievousWoundSpecs`, Lone-Lands unlock trigger, gather/farm yield balance pass)
